...
function getOpenClawProviderBridge() {
    const summary = getProviderSummary();
    const provider = String(summary?.chatProvider || '').trim().toLowerCase();
    const modelId = String(summary?.models?.[provider]?.chat || summary?.selectedModel?.chat || '').trim();
    const alias = String(summary?.selectedModel?.displayName || modelId).trim() || modelId;
    const env = {};
    const providers = {};
    ...
    if (provider === 'openrouter') {
        ...
        const baseUrl = String(process.env.OPENROUTER_BASE_URL || '').trim();
        if (baseUrl) {
        ...
        const apiKey = String(process.env.INCEPTION_API_KEY || process.env.IU_BOOTSTRAP_INCEPTION_API_KEY || '').trim();
        if (apiKey) {
            env.INCEPTION_API_KEY = apiKey;
        }
        providers['iu-inception'] = {
            baseUrl: String(process.env.INCEPTION_BASE_URL || 'https://api.inceptionlabs.ai/v1').trim() || 'https://api.inceptionlabs.ai/v1',

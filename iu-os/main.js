...
const OpenClawCliBrowserClient = require('./OpenClawCliBrowserClient');
...
let resolveActionExecutorsReady = null;
const actionExecutorsReadyPromise = new Promise((resolve) => {
    resolveActionExecutorsReady = resolve;
});
const openClawGatewaySupervisor = new OpenClawGatewaySupervisor({
    createClient: (options) => new OpenClawCliBrowserClient(options),
    log: (message) => LoggingSwitch.execution('OpenClaw', message),
});
const openClawSupervisorBridge = new OpenClawSupervisorBridge({
    ...
    log: (message) => LoggingSwitch.execution('OpenClawSupervisor', message),
});
...
const knowledgeService = new KnowledgeService({
    ...
    onChange: (change) => {
        ...
        syncKnowledgeStateToPhoneBridge(change);
    }
});
...
function isRetryablePromptRuntimeError(error) {
...
async function executePromptRuntimeActionTool({ name, args = {}, runId = '', source = 'prompt_agent_runtime', reason = 'Generated by AgentRuntime' } = {}) {
    if (isManagedActionToolName(name)) {
        ...
        if (!parsed.goal || !parsed.app || !parsed.stepsHint) {
            return { ok: false, error: 'Managed action is missing goal, app or steps_hint' };
        }
        ...
        console.log('🧠 [PromptAgent] Prepared runtime action request:', {
            runId,
            requestId,
            app: parsed.app,
            executor: parsed.executor,
            goal: safeSliceText(parsed.goal, 180),
            source,
            reason: safeSliceText(reason, 180)
        });
        ...
        LoggingSwitch.uiux('prompt_agent', 'runtime_action_confirm_requested', {
            ...
            app: parsed.app,
            executor: parsed.executor,
            goalPreview: safeSliceText(parsed.goal, 160)
        });

        return {
            ok: true,
            summary: `Prepared action in ${parsed.app || 'computer'} via ${parsed.executor}`,
            detail: safeSliceText(parsed.goal || parsed.stepsHint || 'Acción lista para confirmar', 180),
            action: {
                type: 'managed_action',
                requestId,
                app: parsed.app,
...
async function runPromptAgentSession({
    prompt,
    runId,
    emit = () => {},
    source = 'prompt_chat',
    ...
    const recent = contextManager.getHistoryForAPI(10);
    const relevantContext = await contextManager.getRelevantContext(cleanPrompt);
    const learnedWorkflows = LearningAgent.findRelevantWorkflows(cleanPrompt, 3);
    const startedAt = Date.now();
    const runtimeEmit = (entry = {}) => {
        emit(entry);
        const payload = {
            runId,
            timestamp: Date.now(),
        ...
        for (const target of mirrorTargets) {
            try {
                if (target && !target.isDestroyed()) {
                    target.send('prompt-agent-progress', payload);
                }
            } catch (_) {
                // Ignore detached windows.
            }
        }
    };
    const runtimeOptions = {
        ...
        longTerm: relevantContext?.longTerm || '',
        learnedWorkflows,
        importTargetMetaId: String(importTargetMetaId || '').trim(),
        emit: runtimeEmit
    };
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            result = await promptAgentRuntime.runPromptChat(runtimeOptions);
            lastError = null;
            break;
        } catch (error) {
            lastError = error;
            const retryable = isRetryablePromptRuntimeError(error);
            LoggingSwitch.execution('PromptAgent', `Runtime error on attempt ${attempt}/${maxAttempts}: ${error?.message || error}`);
            ...
            runtimeEmit({
                ...
                phase: 'retrying',
                visibility: 'public',
                message: 'Se cayó la conexión del modelo. Reintentando...'
            });
    ...
    if (result?.success) {
        contextManager.addMessage('user', cleanPrompt, source);
        if (String(result.assistantReply || '').trim()) {
            contextManager.addMessage('assistant', result.assistantReply, source);
        }
        const actions = Array.isArray(result?.changes?.actions) ? result.changes.actions : [];
        for (const action of actions) {
            const summary = action?.type === 'managed_action'
                ? `Acción preparada: ${action.goal || 'Acción en computador'}`
                : action?.type === 'reminder'
                    ? `Recordatorio preparado: ${action.task || 'Recordatorio'}`
                    : action?.type === 'play_agario'
                        ? 'Agar.io preparado'
                        : 'Acción preparada';
            contextManager.addMessage('tool', summary, 'action_result', {
    ...
    LoggingSwitch.uiux('prompt_agent', 'runtime_session_completed', {
        runId,
        source,
        durationMs: Date.now() - startedAt,
        success: Boolean(result?.success),
        assistantReplyPreview: safeSliceText(result?.assistantReply || '', 140)
    });
...
ipcMain.handle('prompt-agent-run', async (event, payload = {}) => {
    ...
    try {
        const result = await runPromptAgentSession({
            prompt,
            runId,
            source: 'prompt_chat',
            emit: (entry = {}) => {
...
ipcMain.handle('knowledge-preview-openclaw', async () => {
    return OpenClawBridge.scanOpenClawWorkspace();
});

ipcMain.handle('knowledge-import-openclaw', async (event, payload = {}) => {
    const targetMetaId = String(payload?.metaId || '').trim();
    const targetMeta = targetMetaId ? knowledgeService.getMetas().find((meta) => meta.id === targetMetaId) : null;
    const runId = `openclaw_import_${Date.now()}_${Math.random().toString(16).slice(2, 6)}`;
    ensurePromptRunChatWindow(runId);
    const prompt = [
        'Integra el conocimiento completo de OpenClaw en Ü usando las tools de OpenClaw disponibles.',
        'Quiero que la organización la hagas de forma inteligente, no como una copia ciega.',
        'Objetivos:',
        '- inspecciona primero los documentos locales de OpenClaw',
        '- usa list_openclaw_documents y get_openclaw_document para entender la estructura antes de importar',
        '- usa import_openclaw_documents para crear o actualizar las notas provenientes de OpenClaw',
        '- importa los markdowns como notas de Ü evitando duplicados',
        '- crea o actualiza metas cuando tenga sentido para organizar ese conocimiento',
        '- importa también los markdowns de workspace/memory como notas si existen',
        '- sincroniza workspace/memory con la memoria semántica de Ü usando sync_openclaw_memory',
        '- mantén la respuesta final breve',
        targetMeta
            ? `Puedes usar la meta "${targetMeta.title || 'Meta seleccionada'}" (${targetMeta.id}) como punto de entrada o contenedor inicial, pero no fuerces todo dentro de una sola meta si conviene crear estructura adicional.`
            : 'No hay una meta objetivo obligatoria; organiza la estructura donde tenga más sentido.'
    ].join('\n');

    try {
        if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('prompt-agent-progress', {
                runId,
                timestamp: Date.now(),
                type: 'user_message',
                phase: 'input',
                visibility: 'public',
                message: 'Importa el conocimiento completo de OpenClaw a mi sistema y organízalo inteligentemente.'
            });
        }
        ...
        const result = await runPromptAgentSession({
            ...
            importTargetMetaId: targetMetaId,
            mirrorTargets: [
        ...
        return {
            success: false,
            runId,
            error: error?.message || 'No se pudo importar OpenClaw',
            assistantReply: '',
            state: knowledgeService.getKnowledgeState()
        };
    }
...
ipcMain.handle('knowledge-import-openclaw-direct', async () => {
    const result = await OpenClawBridge.importOpenClawWorkspace({ knowledgeService });
    return {
        ...result,
        state: knowledgeService.getKnowledgeState()
    };
...
async function waitForScreenAgentIdle(timeoutMs = 2000) {
    if (!screenAgent && !openClawExecutor) return;
    const start = Date.now();
    while (((screenAgent && screenAgent.isRunning) || (openClawExecutor && openClawExecutor.isRunning)) && (Date.now() - start) < timeoutMs) {
        await new Promise(resolve => setTimeout(resolve, 50));
    }
}

async function waitForManagedExecutorReady(executor, timeoutMs = 15000) {
    const normalized = normalizeManagedExecutor(executor);
    if (!normalized) return;

    const alreadyReady = normalized === MANAGED_EXECUTOR_OPENCLAW
        ? Boolean(openClawExecutor)
        : Boolean(screenAgent);
    if (alreadyReady) return;

    await Promise.race([
        actionExecutorsReadyPromise.catch(() => {}),
        sleep(timeoutMs)
    ]);
}

function resolveManagedExecutor(executor) {
    const normalized = normalizeManagedExecutor(executor);
    if (!normalized) {
        throw new Error('Managed action is missing a valid executor');
    }
    if (normalized === MANAGED_EXECUTOR_OPENCLAW) {
        if (!openClawExecutor) {
            throw new Error('OpenClaw gateway executor not ready');
        }
        return {
            key: MANAGED_EXECUTOR_OPENCLAW,
            label: 'OpenClawGatewayExecutor',
            instance: openClawExecutor,
            logEvent: 'managed_action_delegated_to_openclaw_executor'
        };
    }
    if (!screenAgent) {
        throw new Error('IU desktop executor not ready');
    }
    return {
        key: MANAGED_EXECUTOR_IU_DESKTOP,
        label: 'ScreenAgent',
        instance: screenAgent,
        logEvent: 'managed_action_delegated_to_screen_agent'
    };
}

async function startManagedExecution(goal, app, stepsHint, executor, options = {}) {
    await waitForManagedExecutorReady(executor);
    const resolvedExecutor = resolveManagedExecutor(executor);
    const executorLabel = resolvedExecutor.label;
    const executorInstance = resolvedExecutor.instance;

    console.log('🚀 [Action] startManagedExecution requested:', {
        goal: safeSliceText(goal, 180),
        app: app || '',
        executor: resolvedExecutor.key,
        stepsHint: safeSliceText(stepsHint, 180),
        source: options.source || 'unknown',
        sessionId: options.sessionId || null
    });
    LoggingSwitch.uiux('prompt_agent', 'managed_action_start_requested', {
        app: String(app || ''),
        executor: resolvedExecutor.key,
    ...
    if (!session) {
        ...
        session = executionSessions.markRunning(session.id, {
            goal,
            app,
            executor: resolvedExecutor.key,
            stepsHint
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('action-started', { goal, app, executor: resolvedExecutor.key, sessionId: session.id });
        mainWindow.webContents.send('action-status', {
            phase: 'starting',
            status: 'starting',
            goal,
            app,
            step: 'Iniciando la ejecución...'
        });
    }

    let result;
    try {
        console.log(`🚀 [Action] Delegating execution to ${executorLabel}:`, {
            sessionId: session.id,
            app: app || '',
            executor: resolvedExecutor.key,
            goal: safeSliceText(goal, 180)
        });
        LoggingSwitch.uiux('prompt_agent', resolvedExecutor.logEvent, {
            app: String(app || ''),
            executor: resolvedExecutor.key,
            sessionId: session.id,
            goalPreview: safeSliceText(goal, 160)
        });
        result = await executorInstance.executeAction(goal, app, stepsHint, { sessionId: session.id });
        console.log(`✅ [Action] ${executorLabel} finished execution:`, {
            sessionId: session.id,
            success: !!result?.success,
            awaitingUserInput: !!result?.awaitingUserInput,
            aborted: !!result?.aborted,
            error: result?.error || '',
            summary: safeSliceText(result?.summary || '', 180)
        });
        LoggingSwitch.uiux('prompt_agent', 'managed_action_finished', {
            sessionId: session.id,
            success: !!result?.success,
            awaitingUserInput: !!result?.awaitingUserInput,
            aborted: !!result?.aborted,
            error: safeSliceText(result?.error || '', 140),
            summary: safeSliceText(result?.summary || '', 160)
        });
    } catch (error) {
        console.error(`❌ [Action] Managed ${resolvedExecutor.key} action crashed:`, error);
        LoggingSwitch.uiux('prompt_agent', 'managed_action_crashed', {
            sessionId: session?.id || null,
            error: safeSliceText(error?.message || String(error || ''), 160)
        });
        result = {
            success: false,
            error: error?.message || 'Managed screen action crashed'
        };
        if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('action-status', {
                phase: 'error',
...
async function executeFromCommandHoldTranscript(transcript) {
    ...
    if (commandHoldOverride.interruptedFlowContext || resumableSession) {
        ...
        if (continuation) {
            await startManagedExecution(continuation.goal, continuation.app, continuation.stepsHint, continuation.executor || MANAGED_EXECUTOR_IU_DESKTOP, {
    ...
    await startManagedExecution(plan.goal, plan.app, plan.stepsHint, plan.executor || MANAGED_EXECUTOR_IU_DESKTOP, {
        source: 'command_hold_replan'
    });
}
...
app.whenReady().then(async () => {
    ...
    try {
        ...
        browserCoreInitError = String(error?.message || error || '').trim();
        console.error('❌ Failed to initialize browser core:', browserCoreInitError);
    }

    let browserAgentInitError = '';
    try {
        browserAgent = new BrowserAgent(mainWindow, {
            browserCoreClient,
            backendInitError: browserCoreInitError
        });
        ...
        console.log('🌐 [Main] BrowserAgent bootstrap completed');
    } catch (error) {
        browserAgent = null;
        browserAgentInitError = String(error?.message || error || '').trim();
        console.error('❌ Failed to initialize BrowserAgent:', browserAgentInitError);
    }
    ...
    if (brain) {
        brain.setExecutionDispatcher(startManagedExecution);
        brain.setExecutionBusyChecker(() => Boolean(screenAgent?.isRunning || openClawExecutor?.isRunning));
    }
    if (typeof resolveActionExecutorsReady === 'function') {
        resolveActionExecutorsReady({
            screenAgentReady: Boolean(screenAgent),
            openClawExecutorReady: Boolean(openClawExecutor)
        });
        resolveActionExecutorsReady = null;
    }
...
app.on('before-quit', () => {
    ...
    openClawGatewaySupervisor.shutdown();
});
...
ipcMain.handle('execute-explicit-action', async (event, userText) => {
    ...
    if (!screenAgent && !openClawExecutor) {
    ...
    try {
        ...
        if (mainWindow) {
            mainWindow.webContents.send('action-confirm-request', {
                ...
                executorReason: plan.executorReason,
...
ipcMain.handle('confirm-action', async (event, data) => {
    ...
    LoggingSwitch.uiux('prompt_agent', 'confirm_action_received', {
        requestId: data?.requestId || null,
        source: String(data?.source || 'unknown'),
        app: String(data?.app || ''),
        executor: String(data?.executor || ''),
        goalPreview: safeSliceText(data?.goal || '', 160)
    });
    ...
    if (!screenAgent && !openClawExecutor) {
        return { success: false, error: 'Action executors not ready' };
    }

    // Start execution
    // data: { goal, app, stepsHint, ... }
    console.log('✅ [Action] Dispatching confirmed action to managed execution:', {
        requestId: data?.requestId || null,
        app: data?.app || '',
        executor: data?.executor || '',
        goal: safeSliceText(data?.goal || '', 180)
    });
    LoggingSwitch.uiux('prompt_agent', 'confirm_action_dispatching', {
        requestId: data?.requestId || null,
        app: String(data?.app || ''),
        executor: String(data?.executor || ''),
        goalPreview: safeSliceText(data?.goal || '', 160)
    ...
    startManagedExecution(
        ...
        data.app,
        data.stepsHint,
        data.executor || MANAGED_EXECUTOR_IU_DESKTOP,
        { source: 'confirm_action' }
    ).catch((error) => {
        console.error('❌ [Action] confirm-action launch failed:', error);
        if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('action-status', {
                phase: 'error',
                status: 'error',
                error: error?.message || 'No se pudo iniciar la acción'
            });
        }
    });
...
function syncKnowledgeStateToPhoneBridge(change = {}) {
    const source = String(change?.source || '').trim().toLowerCase();
    if (source === 'phone_sync' || source === 'remote_sync') return;

    const state = {
        ...knowledgeService.getKnowledgeState(),
        updatedAt: Date.now()
    };
...
async function handlePhoneBridgeMessage(msg) {
    ...
    switch (msg.type) {
        ...
        case 'knowledge_state_sync':
            await handlePhoneKnowledgeSync(msg.payload);
            break;
...
async function handlePhoneKnowledgeSync(payload) {
    const state = payload?.state;
    if (!state || typeof state !== 'object') return;
    knowledgeService.importKnowledgeState(state, { source: 'phone_sync' });
}
...
async function handlePhoneChat(payload) {
    ...
    try {
        ...
        // Check for action
        if (actionIntent?.kind === 'action' && actionIntent.action?.type === 'managed_action') {
            ...
            phoneBridgeSend({
                ...
                payload: {
                    ...
                    action: {
                        ...
                        executor_reason: action.executorReason,
            ...
            if (mainWindow) {
                mainWindow.webContents.send('action-confirm-request', {
                    ...
                    app: action.app,
                    executor: action.executor,
                    executorReason: action.executorReason,

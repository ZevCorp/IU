...
const refs = {
  ...
  launcherDock: document.querySelector('.launcher-app-dock')
};
...
function resolveUiTheme(baseTheme) {
  const normalized = normalizeTheme(baseTheme);
  if (window.iuOS?.platform === 'android') {
    return normalized;
  }
  return getInverseTheme(normalized);
}

function applyResolvedTheme(baseTheme) {
  document.documentElement.setAttribute('data-inverse-theme', resolveUiTheme(baseTheme));
}
...
function bindEvents() {
  ...
  refs.noteBody.addEventListener('input', () => {
    const tab = getActiveTab();
    if (tab) tab.body = refs.noteBody.value || '';
    autoResizeNoteBody();
    saveTabSoon();
    renderNoteMarkers();
  });
  ...
  refs.launcherDock?.querySelector('[data-dock-action="new-note"]')?.addEventListener('click', async () => {
    const created = await window.uChat.createTab({ templateId: 'blank', title: '', source: 'chat_window' });
    applySnapshot(created?.state || null, { syncEditor: true });
    setMode('notes');

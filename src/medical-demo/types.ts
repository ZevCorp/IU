/**
 * MedEMR - Type Definitions for UI Formalization
...
/**
 * WebSocket message types between Render and Jetson
 */
export type JetsonMessage =
    | { type: 'ping' }
    | { type: 'pong' }
    | { type: 'solve'; grid: number[]; width: number; height: number }
    | { type: 'solution'; path: [number, number][]; success: boolean }
